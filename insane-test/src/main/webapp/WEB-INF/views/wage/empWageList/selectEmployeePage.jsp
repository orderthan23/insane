...
<body>
			...
			<div id="tab-2" class="tab-content">
				...
				<table id="provHistList">
					<thead>
						<tr>
							<th>연</th>
							<th><select id="yearSelect" onchange="selectProvYearMonth()">
					...
					<tbody>
						...
						<c:forEach var="prov" items="${provHistList}">
							<tr>
								<td id="listProvisionName">${ prov.provisionName }</td>
								<td id="listProvisionType">${ prov.provisionType }</td>
								<td id="listAmount">${ prov.amount }</td>
								<td id="listPayDate">${ prov.payDate }</td>
							</tr>
						</c:forEach>
					</tbody>
	...
	<script>
		...
		/* 선택한 날짜와 사원으로 지급 내역 조회 */
		function selectProvYearMonth() {
			var yearSelectIdx = document.getElementById("yearSelect").selectedIndex;

			var year = $("#yearSelect option:selected").val();
			var month = $("#monthSelect option:selected").val();
			var empNoSeq = $("#hiddenEmpSeq").val();
			//var provHistDateList = new Array();
			...
			/* $.ajax({
				type : "POST",
				url : "selectProvHistDateList.sal",
				data : {
					empNoSeq : empNoSeq,
					year : year,
					month : month,
					provHistDateList : provHistDateList
				}
			}); */
