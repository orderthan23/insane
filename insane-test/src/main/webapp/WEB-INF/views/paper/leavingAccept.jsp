...
<body>
	...
	<aside>
			...
			<div id="sideMenu" align="center">
				...
				<br>
				<a style="background:#524FE3; text-decoration:none; color:white;" href="requestPage.cr?type=퇴직">퇴직 증명서</a>
				<br>			
	...
	<div id="section">
		...
		<div class="paperArea" align="center">
			...
			<input type="hidden" name="pageType" value="퇴직">
			<c:if test="${ !empty requestScope.crtfc.crtfcSeq}">
			...
			<tr>
				<input type="hidden" name="empSeq" value="${ requestScope.crtfc.empNoSeq }">
				<td>성명</td>
			...
			<tr>
				...
				<c:if test="${ !empty requestScope.crtfc.issueDate }">
					<td><input type="date" value="${ requestScope.crtfc.issueDate }" readonly></td>
				</c:if>
				<c:if test="${ empty requestScope.crtfc.issueDate }">
			...
			<tr>
				<td><span>용도</span></td>
				<td colspan="3"><input type="text" value="${ requestScope.crtfc.issueReason }" readonly></td>
			</tr>
			<tr>
				<td>발급자</td>
			...
			<tr>
				<td>퇴사사유</td>
				<td colspan="3"><textarea cols="50" rows="8" style="resize:none;" readonly></textarea></td>
			</tr>
		</table>			
		<br><br>
		<c:if test="${fn:contains(auth,'CT2')}">
			<c:if test="${ requestScope.crtfc.processStatus eq 'N' }">
				<button class="btnAccept" type="button" onclick="acceptClick();">발급</button>	
