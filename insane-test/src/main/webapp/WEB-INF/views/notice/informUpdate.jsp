...
<body>
	...
	<c:if test="${!fn:contains(auth,'S2') }">
			<c:redirect url="/errorPage.mp" context="/insane"> <c:param name="msg" value="권한이 없습니다."/> </c:redirect>
	</c:if>
	<div class="section">
		<br>
		<div class="bluerect"></div><span id="pageTitle">전달사항 수정</span><br><br>
		<hr>
		<br><br>
		<form action="update.no" method="post" enctype="multipart/form-data">
		<input type="hidden" id="boardSeq" name="boardSeq" value="${ requestScope.notice.boardSeq }">
		<input type="hidden" class="type" name="type" value="소식">
		<div class="boardArea">
			<table id="boardTable">
				<thead>
					<th width="100">제목</th> 
					<th colspan="2"><input type="text" name="boardTitle" value="${ requestScope.notice.boardTitle }" style="width:300px"></th>
				</thead>
				<tbody>
					<tr>
						<td>공개여부</td>
						<td><input type="radio" id="public" name="access" value="공개" ${ requestScope.notice.status != null && requestScope.notice.status eq "공개" ? "checked" : ""}> <label for="public">공개</label></td>
						<td><input type="radio" id="private" name="access" value="비공개" ${ requestScope.notice.status != null && requestScope.notice.status eq "비공개" ? "checked" : ""}> <label for="private">비공개</label></td>
					</tr>
					<tr>
						<td colspan="3"><div id="customBoard" style="text-align:left;" onclick="txtAreaClick();">
							<cite contenteditable="true" class="textArea">
					...
					<tr>
						<td>파일첨부</td>
						<td><input id="fileName" name="attachFile" type="text" value="${ requestScope.attachment.originName }" readonly></td>
				...
				<c:if test="${fn:contains(auth,'S2')}">
					<button id="btnUpdate" onclick="insertCheck();">수정하기</button>
				</c:if>
				<button id="btnCancle" type="button" onclick="goToDetail();">취소</button>			
			<br><br><br><br><br>
		</div>
		</form>
	</div>
	...
	function goToDetail(){
		var num = $("#boardSeq").val();//$(this).parent().children("#boardSeq").val();
