...
<body>
					...
					<div id="article1" class="tableWrap">
						<div class="tableInfoArea">
							<table class="headTable">
								<tr>
									<td><span class="tableInfoHeader">부서</span></td>
									<td><span>${fn:length(departmentList) }개</span></td>
						...
						<table class="tableType1 table">
							...
							<tbody>
								...
								<c:forEach var="i"  begin="0" end="${fn:length(departmentList)-1 }">
								<tr class="deptList">
									<td class="deptCodeSeq">${departmentList[i].deptCodeSeq }</td>
									<td>${departmentList[i].deptName }</td>
								</tr>
								</c:forEach>
								</c:if>
							</tbody>
	...
	<input type="hidden" id="hiddenDept">
...
<script type="text/javascript">
	...
	$('.deptList').click(function(){
		...
		$('.deptList').not(this).css('background','white');
		var seq = $(this).find('.deptCodeSeq').text();
	...
	$('#selectBar').change(function(){
		var deptNumber2 = $('#hiddenDept').val();
		if(deptNumber2 == "" || deptNumber2 == null){
			alert("권한을 먼저 선택해주세요 ");
		}else{
