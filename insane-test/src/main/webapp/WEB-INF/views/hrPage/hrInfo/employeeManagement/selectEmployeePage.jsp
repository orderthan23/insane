	...
	<div id="contentWrapper">
		...
		<div id="contentArea">
			...
			<div id="contentHeader">
				<div id="bluebox"></div>
				<label>사원 정보 조회</label>
			</div>	<!-- contentHeader End -->
			<div id="headerLine"></div>
				<table id="btnArea">
					<tr>
						<td><span id="subTitle">기초 정보 조회</span></td>
						<td style="text-align: right;" >
							<c:if test="${fn:contains(auth,'EL2') }">
								<span id="empUpdateBtn">+&nbsp;&nbsp;수정하기</span>
						    </c:if>
						</td>
					</tr>
				</table>
			...
			<div id="tableArea">
				<table id="basicEmpInfo">
					<tr>
						...
						<td class="tdType2">
							<span class="column2">
								<input type="text" name="empNameKor" readonly="readonly" value="${ hmap.emp.empNameKor }">
							</span>
						</td>
					...
					<tr>
						...
						<td class="tdType2"><span class="column2"><input type="text" name="empNameEng" readonly="readonly" value="${ hmap.emp.empNameEng }"></span></td>
					...
					<tr>
						...
						<td class="tdType2" colspan="3">
							<span class="column2" style="width: 415px;">
								<input type="text" id="empAddress" name="empAddress" style="width: 384px;" readonly="readonly" value="${ hmap.emp.empAddress }">
							</span>
						</td>
					</tr>
				</table>
...
<script>
	...
	$(function() {
		...
		$("#empBirthday").attr("readonly", "readonly");
		$("#lunarSor_S").attr("readonly", "readonly").attr("onclick", "return(false)");
		$("#lunarSor_L").attr("readonly", "readonly").attr("onclick", "return(false)");
		$("#gender_m").attr("readonly", "readonly").attr("onclick", "return(false)");
		$("#gender_f").attr("readonly", "readonly").attr("onclick", "return(false)");
		$("#empHomeTel").attr("readonly", "readonly");
		$("#empPhone").attr("readonly", "readonly");
		$("#empOfficeTel").attr("readonly", "readonly");
		$("#empForm").attr("readonly", "readonly");
		$("#empOccupation").attr("readonly", "readonly");
		$("#empSpot").attr("readonly", "readonly");
		$("#empProject").attr("readonly", "readonly");
	});
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data != null){
					...
					$("#hltIns").children("td").eq(4).html("");
					$("#accIns").children("td").eq(4).html("");
					$("#empIns").children("td").eq(4).html("");
					$("#ntnIns").children("td").eq(4).html("");
				}
		...
		$.ajax({
			...
			success: function(data) {
				...
				if(data.military != null){
					...
					$("#endsType").attr("readonly", "readonly");
					$("#milPosition").attr("readonly", "readonly");
					$("#milSpecies").attr("readonly", "readonly");
					$("#lastClass").attr("readonly", "readonly");
					$("#servStartDate").attr("readonly", "readonly");
					$("#servEndDate").attr("readonly", "readonly");
					$("#exmpReason").attr("readonly", "readonly");
				}
			},
			error: function() {
				console.log("error");
	...
	$("#rpInfo").click(function() {
		...
		$("#insertRewardPunishInfoBtn").hide();
	...
	$("#empUpdateBtn").click(function() {
		var num = $("#basicEmpInfo #empNoSeq").val();
		
		location.href="showUpdateEmpPage.em?num=" + num;
	});
